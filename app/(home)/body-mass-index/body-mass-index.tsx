...
const BodyMassIndex = () => {
  ...
  return (
    <form
      ...
      className="border rounded-2xl sm:rounded-xl shadow-lg bg-white"
    >
      ...
      <div className="flex flex-col gap-6 pt-6 pb-8 px-6 sm:pt-8 sm:pb-10 sm:px-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          <Input
            ...
            value={weight ?? ""}
            onChange={(event) =>
              setWeight(event.target.value ? Number(event.target.value) : null)
            }
            className={cn(errorMessage ? "border-red-500" : "border-gray-300")}
          />
        </div>
        ...
        {bmi && (
          <div className="flex flex-col items-center gap-3 mt-4">
            <p className="text-xl font-semibold">
              Chỉ số BMI của bạn:{" "}
              <span className="text-[#00B16A]">{bmi.toFixed(2)}</span>
            </p>
            <p className="text-lg font-semibold text-[#00B16A]">{bmiResult}</p>
          </div>
        )}
      </div>
    </form>
