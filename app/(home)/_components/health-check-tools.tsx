...
const Tool = ({ id, title, image }: IProps) => (
  <Link key={id} href="/" prefetch={false} className="flex flex-col items-center gap-6">
    <div className="relative w-[70px] h-[70px]">
      <Image loading="lazy" src={image} alt="Tool" fill className="w-full h-full object-cover" />
    </div>
...
const HealthCheckTools = () => {
  return (
    <div className="flex flex-col gap-12 p-8 border rounded-2xl sm:rounded-xl shadow-lg">
      <h1 className="text-xl font-bold text-center">Các công cụ kiểm tra sức khỏe khác</h1>
      <div className="flex flex-col gap-8">
        <div className="grid grid-cols-3 gap-6">
          {healthCheckTools.map(({ id, title, image }: HealthCheckTool) => (
